...
const tsconfig = JSON.parse(
  readFileSync(resolve(process.cwd(), './tsconfig.json')).toString(),
);
...
function prepareDist() {
  postBuildScript({
    ...
    onDone: (_, pckgJson) => {
      const dirLibsPaths = (
        Object.entries(tsconfig.compilerOptions.paths) as unknown as [
          string,
          string[],
        ]
      )
      ...
      Object.keys(pckgJson.data.exports).forEach((exportPath) => {
        ...
        if (!pckgJson.data.exports[exportPath].require) {
          pckgJson.data.exports[exportPath].require = pckgJson.data.exports[
            exportPath
          ].import.replace('.js', '.cjs');
        }
