...
If you use [`QueryClient`](/v6/api/QueryClient) hooks, `onQueryInit` runs after `onInit`.
...
[_Can be specified using `QueryClient`_](https://js2me.github.io/mobx-tanstack-query/v6/api/QueryClient.html#queryfeatures)
...
If you work with [`QueryClient`](/v6/api/QueryClient) then calling `mount()` is not needed.
