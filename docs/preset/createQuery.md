...
TanStack Query expects the [`QueryClient`](/api/QueryClient) to be **mounted** so observers, refetch scheduling, and related behavior work as intended.

After the `Query` is constructed, preset `createQuery` calls `mountQueryClientOnce` for the **effective client** attached to that query (the default [`queryClient`](/preset/queryClient) from the preset, or the `queryClient` you pass in options). Each client instance is only mounted once across the process (`WeakSet` inside [`mountQueryClientOnce`](/src/utils/mount-query-client-once.ts)), so many `createQuery` calls against the same client do not stack `mount()` work.

If you use [`new Query(...)`](/api/Query#usage) without this preset, you must arrange `queryClient.mount()` yourself when your setup requires it, as described in the [`Query`](/api/Query) docs.
